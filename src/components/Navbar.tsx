          ...
          <div className="hidden md:flex items-center gap-4 lg:gap-6">
            ...
            <form onSubmit={handleSearch} className="relative group">
              <input
                ...
                className="w-32 lg:w-48 bg-zinc-900/40 border border-white/5 rounded-2xl py-2 pr-10 pl-4 text-xs font-medium focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:bg-zinc-900/80 transition-all placeholder:text-zinc-600 focus:w-64 text-right"
              />
              <Search className="absolute right-3.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-zinc-600 group-focus-within:text-emerald-500 transition-colors" />
            </form>
            ...
            {user ? (
              <div className="flex items-center gap-3 lg:gap-4">
                <Link to="/notifications" className="p-2.5 text-zinc-400 hover:text-white hover:bg-white/5 rounded-2xl transition-all relative">
                  ...
                  <div className="absolute top-2 left-2 w-1.5 h-1.5 bg-emerald-500 rounded-full border-2 border-zinc-950" />
                </Link>
                
                {isAdmin && (
                  <Link to="/admin" className="p-2.5 bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500/20 rounded-2xl transition-all" title="لوحة التحكم">
              ...
              <button
                onClick={handleLogin}
                className="px-6 py-2.5 bg-white text-black text-[11px] font-black rounded-xl hover:scale-105 active:scale-95 transition-all uppercase tracking-widest shadow-xl shadow-white/5"
              >
                دخول
              </button>
            )}
          ...
          >
            <div className="px-4 py-6 space-y-6">
              <form onSubmit={handleSearch} className="relative">
                <input
                  type="text"
                  placeholder="البحث عن السلاسل..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="w-full bg-zinc-900 border border-white/10 rounded-xl py-3 pr-12 pl-4 text-sm focus:outline-none focus:border-emerald-500/50 text-right"
                />
              ...
              {user ? (
                <div className="flex items-center justify-between p-4 bg-zinc-900 rounded-2xl">
                  <Link to="/profile" onClick={() => setIsMenuOpen(false)} className="flex items-center gap-3">
                    ...
                    />
                    <div>
                      <p className="text-sm font-bold">{profile?.username || user.user_metadata?.full_name || 'مستخدم'}</p>
                      <p className="text-[10px] text-zinc-550 font-bold uppercase tracking-widest text-right">عرض الملف الشخصي</p>
                    </div>
                  </Link>
